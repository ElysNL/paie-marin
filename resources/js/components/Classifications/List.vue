<template>
  <div>
    ...
    <div class="mb-6 flex items-center justify-between">
      ...
      <router-link to="/classifications/create">
        <AppButton>Nouvelle classification</AppButton>
      </router-link>
    </div>
    ...
    <AppCard>
      ...
      <AppTable :columns="columns" :rows="store.classifications" row-key="id">
        ...
        <template #cell(actions)="{ row }">
          <div class="flex items-center gap-1">
            <router-link :to="`/classifications/${row.id}/edit`">
      ...
      <AppEmpty v-if="!loading && store.classifications.length === 0" message="Aucune classification enregistrée." />
      <AppLoading v-if="loading" message="Chargement des classifications…" />
    </AppCard>
  </div>
</template>
...
const fetchData = async (page = 1) => {
  loading.value = true;
  try {
    await store.fetchClassifications(page);
  } finally {
    loading.value = false;
  }
};
...
const remove = async (row) => {
  if (!confirm('Voulez-vous supprimer cette classification ?')) return;
  await store.deleteClassification(row.id);
};
