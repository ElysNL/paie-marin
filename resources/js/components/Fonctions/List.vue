<template>
  <div>
    ...
    <div class="mb-6 flex items-center justify-between">
      <h1 class="text-2xl font-bold">Fonctions</h1>
      <router-link to="/fonctions/create">
        <AppButton>Nouvelle fonction</AppButton>
      </router-link>
    </div>
    ...
    <AppCard>
      ...
      <AppTable :columns="columns" :rows="store.fonctions" row-key="id">
        ...
        <template #cell(actions)="{ row }">
          <div class="flex items-center gap-1">
            <router-link :to="`/fonctions/${row.id}/edit`">
      ...
      <AppEmpty v-if="!loading && store.fonctions.length === 0" message="Aucune fonction enregistrée." />
      <AppLoading v-if="loading" message="Chargement des fonctions…" />
    </AppCard>
  </div>
</template>
...
const fetchData = async (page = 1) => {
  loading.value = true;
  try {
    await store.fetchFonctions(page);
  } finally {
    loading.value = false;
  }
};
...
const remove = async (row) => {
  if (!confirm('Voulez-vous supprimer cette fonction ?')) return;
  await store.deleteFonction(row.id);
};
