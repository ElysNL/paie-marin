<template>
  <div>
    ...
    <div class="mb-6 flex items-center justify-between">
      <h1 class="text-2xl font-bold">Contrats d'armateur</h1>
      <router-link to="/contrats-armateur/create">
        <AppButton>Nouveau contrat</AppButton>
      </router-link>
    </div>
    ...
    <AppCard>
      ...
      <AppTable :columns="columns" :rows="store.contrats" row-key="id">
        ...
        <template #cell(taux_base)="{ row }">{{ row.taux_base }}</template>
        ...
        <template #cell(actions)="{ row }">
          <div class="flex items-center gap-1">
            <router-link :to="`/contrats-armateur/${row.id}/edit`">
      ...
      <AppEmpty v-if="!loading && store.contrats.length === 0" message="Aucun contrat enregistré." />
      <AppLoading v-if="loading" message="Chargement des contrats…" />
    </AppCard>
  </div>
</template>
...
const fetchData = async (page = 1) => {
  loading.value = true;
  try {
    await store.fetchContrats(page);
  } finally {
    loading.value = false;
  }
};
...
const remove = async (row) => {
  if (!confirm('Voulez-vous supprimer ce contrat ?')) return;
  await store.deleteContrat(row.id);
};
