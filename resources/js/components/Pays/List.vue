<template>
  <div>
    ...
    <div class="mb-6 flex items-center justify-between">
      <h1 class="text-2xl font-bold">Pays</h1>
      <router-link to="/pays/create">
        <AppButton>Nouveau pays</AppButton>
      </router-link>
    </div>
    ...
    <AppCard>
      ...
      <AppTable :columns="columns" :rows="store.pays" row-key="id">
        <template #cell(code)="{ row }">{{ row.code }}</template>
        <template #cell(nom)="{ row }">{{ row.nom }}</template>
        <template #cell(nationalite)="{ row }">{{ row.nationalite }}</template>
        ...
        <template #cell(actions)="{ row }">
          <div class="flex items-center gap-1">
            <router-link :to="`/pays/${row.id}/edit`">
      ...
      <AppEmpty v-if="!loading && store.pays.length === 0" message="Aucun pays enregistré." />
      <AppLoading v-if="loading" message="Chargement des pays…" />
    </AppCard>
  </div>
</template>
...
const remove = async (row) => {
  if (!confirm('Voulez-vous supprimer ce pays ?')) return;
  await store.deletePays(row.id);
};
